...
import { User } from '../services/endpoints/auth'
...
type AuthData = {
  ...
  handleLogin: (email: string, password: string) => Promise<{ logged: boolean}>
  isLogged: boolean
  checkLogged: () => boolean
  logout: () => void
  loading: boolean
  ...
  const handleLogin = async (email: string, password: string) => {
    try {
      ...
      if (userRaw) {
        ...
        localStorage.setItem('user', JSON.stringify(userRaw.user))
